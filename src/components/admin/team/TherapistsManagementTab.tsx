...
export default function TherapistsManagementTab({ 
  data: therapistsData, 
  onUpdate, 
  onUnsavedChanges,
  onRegisterCallback,
  onRegisterCleanupCallback
}: TherapistsManagementTabProps) {
  ...
  const handleSaveSuccess = React.useCallback(async () => {
    ...
    stopTrackingAllImages()
    markAsSaved()
    console.log('Therapists 탭: 삭제 예정 이미지 처리 완료, 이미지 추적 중단 및 저장 완료 표시')
  ...
  const handleDiscardChanges = React.useCallback(async () => {
    ...
    console.log('Therapists 탭: 이미지 정리 완료')
  }, [performCleanup])
  ...
  React.useEffect(() => {
    if (onRegisterCallback) {
      ...
      console.log('Therapists 탭: 저장 성공 콜백 등록')
    }
  }, [onRegisterCallback, handleSaveSuccess])
  ...
  React.useEffect(() => {
    if (onRegisterCleanupCallback) {
      ...
      console.log('Therapists 탭: 정리 콜백 등록')
    }
  }, [onRegisterCleanupCallback, handleDiscardChanges])
  ...
  return (
    <div className="space-y-6">
      {/* 히어로 섹션 */}
      <AdminSection title="히어로 섹션" description="치료·상담사 페이지의 히어로 섹션을 관리합니다.">
        <AdminInput
          label="페이지 제목"
        ...
        <AdminImageUploadField
          ...
          folder="pages/team/therapists/hero"
      ...
      {/* 소개 메시지 */}
      <AdminSection title="소개 메시지" description="치료·상담사 소개 메시지를 관리합니다.">
        <AdminInput
          label="제목"
      ...
      {/* 특징 관리 */}
      <AdminSection title={`특징 관리 (${therapistsData.features?.length || 0}개)`} description="치료·상담사팀의 특징을 관리합니다.">
        <div className="space-y-3">
          {therapistsData.features?.map((feature) => (
        ...
        <div className="space-y-4">
          {therapistsData.members.length === 0 ? (
            <div className="text-center py-8 text-gray-500">
              <FileText className="mx-auto h-12 w-12 mb-2 text-gray-400" />
              <p>등록된 치료사가 없습니다.</p>
              <p className="text-sm">첫 번째 치료사를 추가해보세요.</p>
            </div>
          ) : (
                  ...
                  <div className="space-y-4 mt-4 pt-4 border-t">
                    ...
                    <div className="bg-green-50 rounded-lg p-4">
                      ...
                      <AdminArrayField
                        ...
                        onAdd={(item) => {
                          const newData = JSON.parse(JSON.stringify(therapistsData))
