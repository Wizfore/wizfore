...
export default function AboutPage() {
  ...
  const [tabCleanupCallbacks] = useState<{
    [key in AboutTab]?: () => Promise<void>;
  }>({});

  // 탭별 저장 성공 콜백 등록
  const registerTabCallback = useCallback(
    (tabKey: AboutTab, callback: () => void) => {
  ...
  // 탭별 정리 콜백 등록
  const registerTabCleanupCallback = useCallback(
    (tabKey: AboutTab, callback: () => Promise<void>) => {
  ...
  const {
    ...
    saveData: async (data: AboutData) => {
      ...
      const {
        ...
        updateAdvisorsInfo,
        updateLocationInfo,
      } = await import("@/lib/services/dataService");
      const { facilityService } = await import(
        "@/lib/services/facilityService"
      );
  ...
  const facilityHasChanges = React.useMemo(() => {
    // 다른 탭의 변경사항 확인
    const otherTabsHaveChanges =
      activeTab !== "facilities" ? hasChanges : false;
  ...
  const handleCleanupBeforeLeave = async () => {
    ...
    if (
      hasFacilityChanges &&
      (window as { __facilityCleanup?: () => Promise<void> }).__facilityCleanup
    ) {
      try {
        await (
          window as { __facilityCleanup?: () => Promise<void> }
        ).__facilityCleanup?.();
      ...
      case "director":
        return (
          <DirectorManagementTab
            ...
            onUpdate={(directorData) =>
              setAboutData((prev) => ({ ...prev, director: directorData }))
            }
            onRegisterCallback={(callback) =>
              registerTabCallback("director", callback)
            }
            onRegisterCleanupCallback={(callback) =>
              registerTabCleanupCallback("director", callback)
            }
          />
        );
      case "history":
        return (
          <HistoryManagementTab
            ...
            onUpdate={(historyData) =>
              setAboutData((prev) => ({ ...prev, history: historyData }))
            }
            onRegisterCallback={(callback) =>
              registerTabCallback("history", callback)
            }
            onRegisterCleanupCallback={(callback) =>
              registerTabCleanupCallback("history", callback)
            }
          />
        );
      case "advisors":
        return (
          <AdvisorsManagementTab
            ...
            onUpdate={(advisorsData) =>
              setAboutData((prev) => ({ ...prev, advisors: advisorsData }))
            }
            onRegisterCallback={(callback) =>
              registerTabCallback("advisors", callback)
            }
            onRegisterCleanupCallback={(callback) =>
              registerTabCleanupCallback("advisors", callback)
            }
          />
        );
      case "facilities":
        return (
          <FacilityManagementTab
            ...
            onRegisterCallback={(callback) =>
              registerTabCallback("facilities", callback)
            }
            onRegisterCleanupCallback={(callback) =>
              registerTabCleanupCallback("facilities", callback)
            }
          />
        );
      case "location":
        return (
          <LocationManagementTab
            ...
            onUpdate={(locationData) =>
              setAboutData((prev) => ({ ...prev, location: locationData }))
            }
            onRegisterCallback={(callback) =>
              registerTabCallback("location", callback)
            }
            onRegisterCleanupCallback={(callback) =>
              registerTabCleanupCallback("location", callback)
            }
          />
        );
      default:
