...
export default function TherapyPage() {
    ...
    const fetchTherapyData = async () => {
      try {
        ...
        if (therapyProgram) {
          ...
          setHero(therapyProgram.hero)
          setAboutMessage(therapyProgram.aboutMessage)
          setFeatures([])
        }
        setError(null)
      } catch (err) {
        console.error('Error fetching therapy data:', err)
        setError('치료 프로그램 정보를 불러오는데 실패했습니다.')
        
        // Fallback to default data
        const therapyProgram = defaultSiteData.programs.find(p => p.id === 'therapy')
        if (therapyProgram) {
          setPrograms(therapyProgram.programs || [])
          setHero(therapyProgram.hero || null)
          setAboutMessage(therapyProgram.aboutMessage || null)
          setFeatures([])
        }
      } finally {
        setLoading(false)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-wizfore-warm-brown via-wizfore-coral-primary to-wizfore-soft-pink flex items-center justify-center">
        ...
        >
          ...
          <p className="text-white text-xl font-semibold">치료 프로그램 정보를 불러오는 중...</p>
        </motion.div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* 치료 프로그램 목록 섹션 */}
      <TherapyListSection programs={programs} />
    </div>
  )
}